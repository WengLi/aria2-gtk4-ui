...
extern "C"
{
  ...
  /// @brief Check whether a particular key is being mapped into the HashMap or not.
  /// @param hashmap The hashmap to get from.
  /// @param key The string key to use.
  /// @param len The length of the string key.
  /// @param value if contains key,set value
  /// @return 1:contains key,0:not contains key.
  static int hashmap_contains_key(const struct hashmap_s *const hashmap,
                                  const char *const key,
                                  const unsigned len) HASHMAP_USED;
...
int hashmap_contains_key(const struct hashmap_s *const m, const char *const key,
                         const unsigned len)
{
  unsigned int curr;
  unsigned int i;
